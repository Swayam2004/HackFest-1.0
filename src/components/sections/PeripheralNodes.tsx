...
function ProtocolButton() {
	...
	return (
		<div className="relative mt-4 pt-2 cursor-pointer" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
			{/* Outer red border + chamfer — hard yellow outline pulse on hover */}
			<div
				className="bg-hack-red p-[2px]"
				style={{
					clipPath: hovered ? "polygon(0 0, 100% 0, 100% 100%, 0 100%, 0 0)" : "polygon(16px 0, 100% 0, 100% calc(100% - 16px), calc(100% - 16px) 100%, 0 100%, 0 16px)",
				...
				>
					...
					<div
						...
						style={{
							...
							animation: hovered ? "btn-fill-wipe 0.08s steps(8, end) forwards" : "none",
					...
					<div
						...
						style={{
							...
							zIndex: 2,
							pointerEvents: "none",
							animation: hovered ? "btn-scanbar 0.1s steps(10, end) forwards" : "none",
						...
						{visibleEvents.map((node, i) => (
							...
							>
								...
								<div className="peripheral-card-visual border-b-2 border-hack-red clip-diagonal relative group-hover:border-b-hack-yellow transition-colors">
									...
									{/* Floating ID badge */}
									<div className="absolute bottom-4 left-4 bg-hack-red text-black font-mono font-black px-3 py-1 text-sm md:text-base border border-black shadow-[2px_2px_0_0_#000]">[ {node.id} ]</div>
								</div>
								...
								<div className="p-5 md:p-6 flex flex-col flex-1 gap-4">
									<div className="flex justify-between items-start gap-2">
										<div className="flex flex-col gap-1">
											...
											<span className="font-mono text-[10px] sm:text-xs text-white bg-white/5 px-2 py-0.5 border border-white/10 uppercase backdrop-blur-sm">{node.sys_type}</span>
										</div>

										<div className="flex flex-col gap-1 text-right">
											<span className="font-mono text-[9px] sm:text-[10px] text-hack-red font-bold">REWARD_</span>
											<span className="font-mono font-black text-[10px] sm:text-xs text-black bg-hack-yellow px-2 py-0.5 shadow-[2px_2px_0_0_white]">{node.bounty}</span>
										</div>
									</div>
									...
									<div className="flex gap-3 flex-1">
										...
										<p className="font-mono text-[11px] sm:text-[12px] uppercase text-gray-400 leading-relaxed pt-1">{node.desc}</p>
									</div>
